...
import com.springstudy.demo.beans.sample.*;
...
public class XmlApplicationContextTest {
     ...
     * (3번을 하나의 역할로 판단할 수 있을 것이다. Spring의 경우 "ConfigureListableBeanFactory 인터페이스에
     * 정의하고 있는데, 그 이유는 아직까지 모르겠다. (알게되면 공유 부탁해요!))
     * 4번은 beanDefinition을 관리하고 있는 현재의 역할에서 적절해보이지만
     * DefaultListableBeanFactory라는 이름은 어울리지 않아보인다.
     * 차라리 DefaultListableBeanDefinitionFactory나 register라는 메소드 명에 따라
     * BeanDefinitionRegistry같은 네이밍이 어울린다.
     *
     ...
     *
     * 결론적으로 registerBeanDefinition은 DefaultListableBeanFactory와 AbstractBeanFactory 양쪽 어디에도
     * 속할 이유가 없어보인다.
     ...
     *       앞선 과제에서 추가한 BeanDefinitionRegister와 새로 정의한 BeanDefinitionReader가
     *       DI를 통해 동작하도록 한다.
     *
    ...
    public void successIfBeanInitializedAtFirst() throws InterruptedException {
        XmlApplicationContext xmlBeanFactory = new XmlApplicationContext(ClassLoader.getSystemResourceAsStream("step5.xml"));
        BeanD d = xmlBeanFactory.getBean("BeanD", BeanD.class);
        Thread.sleep(2000);
        BeanE e = xmlBeanFactory.getBean("BeanE", BeanE.class);
    ...
    public void invokeMethodAfterInitializingBean() {
        XmlApplicationContext xmlBeanFactory = new XmlApplicationContext(ClassLoader.getSystemResourceAsStream("step4.xml"));
        BeanC c = xmlBeanFactory.getBean("BeanC", BeanC.class);
        assertTrue(c.flag);
    }
    ...
    public void hierarchicalBeanFactory() {
        XmlApplicationContext parentFactory = new XmlApplicationContext(ClassLoader.getSystemResourceAsStream("step4_parent.xml"));
        XmlApplicationContext childFactory = new XmlApplicationContext(ClassLoader.getSystemResourceAsStream("step4_child.xml"), parentFactory);

        assertNotNull(childFactory.getBean("BeanA"));
        assertEquals(childFactory.getBean("BeanB"), parentFactory.getBean("BeanB"));

        BeanB b = childFactory.getBean("BeanB", BeanB.class);
        assertEquals(b.name, "JH");
        assertEquals(b.age.intValue(), 31);
    }
    ...
    public void registerBeanWithProperty() {
        ...
        XmlApplicationContext beanFactory = new XmlApplicationContext(inputStream);
        BeanB b = beanFactory.getBean("BeanB", BeanB.class);
        assertEquals(b.name, "JH");
        assertEquals(b.age.intValue(), 31);
        b.name = "Java";
        b.age = 22;

        BeanB reloadedB = beanFactory.getBean("BeanB", BeanB.class);
    ...
    public void registerObjectAsSingleTon() {
        ...
        XmlApplicationContext xmlApplicationContext = new XmlApplicationContext(inputStream);
        BeanA a = xmlApplicationContext.getBean("BeanA", BeanA.class);
        assertEquals(a.name, "BeanA");
        a.name = "AA";

        BeanA reloadedA = xmlApplicationContext.getBean("BeanA", BeanA.class);
